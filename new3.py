...
print(util.pytorch_cos_sim(embedding_Devops, model.encode("swift")))
print(util.pytorch_cos_sim(embedding_Devops, model.encode("Python")))
print(util.pytorch_cos_sim(embedding_Devops, model.encode("Docker")))#Spring
print(util.pytorch_cos_sim(embedding_Devops, model.encode("Spring")))#TCP/IP
print(util.pytorch_cos_sim(embedding_Devops, model.encode("TCP/IP")))
...
for i, sentence in enumerate(sentences2):
    plt.text(embeddings_pca[i, 0], embeddings_pca[i, 1], sentence, fontsize=8)
x_var = 'result'
groupby_var = 'prophecy_name'
...
plt.title("UMAP")
def annotate_points(df, ax):
    for index, row in df.iterrows():
        ax.annotate(row['sentence'], (row['x'], row['y']), textcoords="offset points", xytext=(-10,10), ha='center')
annotate_points(df, plt.gca())
plt.show()
